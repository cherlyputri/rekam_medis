...
$pdf->SetX(8);
$pdf->MultiCell(19.5, 0.7, "PEMERINTAHAN KOTA TIDORE KEPULAUAN", 0, 'C');
...
if (!empty($resep_obat)) {
    ...
    foreach ($resep_obat as $data) {
        ...
        $pdf->Cell(3.5, 0.6, $data['kd_resep'], 1, 0, 'C');
        $pdf->Cell(9, 0.6, $data['keluhan'], 1, 0, 'L');
        $pdf->Cell(6, 0.6, 'Rp. ' . number_format($data['subtotal'], 0, ',', '.'), 1, 0, 'C');
