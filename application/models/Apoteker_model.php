...
class Apoteker_model extends CI_Model
{
	...
	{
		return $this->db->get('petugas_obat');
	}

	//fungsi untuk ngitung jumlah data apotekernya
	function jumlahapoteker()
	{
		$query = $this->db->get('petugas_obat'); //pertama bikin variabel query isinya buat ambil data dr tabel petugas obat
		if ($query->num_rows() > 0) { //terus di cek jumlah baris di tabel admin lebih dr 0 apa tidak
			return $query->num_rows(); //kalo lebih dari 0, misalnya jumlah barisnya ada 2 soalnya ada 2 apoteker yg terdaftar nah angka 2 tadi dikembaliin buat ditampilin
		} else {
	...
	public function hapus_data($id_petugas_obat)
	{
		$this->db->where('id_petugas_obat', $id_petugas_obat);
