...
class Pembayaran_model extends CI_Model
{
	...
	//fungsi untuk ngitung jumlah pembayaran
	function jumlahpembayaran()
	{
		$query = $this->db->get('pembayaran'); //pertama bikin variabel query isinya buat ambil data dr tabel pembayaran
		if ($query->num_rows() > 0) { //terus di cek jumlah baris di tabel pembayaran lebih dr 0 apa tidak
			return $query->num_rows(); //kalo lebih dari 0, misalnya jumlah barisnya ada 2 soalnya ada 2 pembayaran yg terdaftar nah angka 2 tadi dikembaliin buat ditampilin
		} else {
	...
	function getAllBayar()
	{
		$this->db->select('*');
		$this->db->from('resep');
		//join adalah penggabungan table yang dilakukan melalui kolom tertentu yang memiliki nilai terkait untuk mendapatkan satu set data dengan informasi lengkap
	...
	function getBayar($where)
	{
		...
		return $query = $this->db->get()->result(); //menampilkan semua data
	}
